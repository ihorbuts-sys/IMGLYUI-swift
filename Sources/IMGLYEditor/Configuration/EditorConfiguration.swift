...
open class EditorConfiguration {
  ...
  /// The `onCheck` handler.
  open var onCheck: OnCheck.Handler? { builder.onCheck }
...
public extension EditorConfiguration {
  ...
  final class Builder {
    ...
    private(set) var onCheck: OnCheck.Handler?
    ...
    /// Sets the `onCheck` handler.
    public func onCheck(_ handler: @escaping OnCheck.Handler) {
      onCheck = handler
    }
...
extension EditorConfiguration {
  ...
  func configure(_ composer: EditorConfigurationComposer) {
    ...
    if let handler = builder.onCheck ?? onCheck { composer.onCheck(handler) }
