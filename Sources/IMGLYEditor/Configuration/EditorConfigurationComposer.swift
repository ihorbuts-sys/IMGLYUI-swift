...
final class EditorConfigurationComposer {
  ...
  private var onCheck: OnCheck.Callback?
  ...
  /// Sets the `onCheck` callback.
  ///
  /// The handler receives the engine, event handler, and an `existing` closure.
  /// Call `existing()` to invoke any previously configured `onCheck` callbacks.
  ///
  /// - Parameter handler: The handler to set.
  func onCheck(_ handler: @escaping OnCheck.Handler) {
    let previous = onCheck
    onCheck = { engine, eventHandler in
      try await handler(engine, eventHandler) {
        try await previous?(engine, eventHandler)
      }
    }
