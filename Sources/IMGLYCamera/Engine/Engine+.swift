...
@_spi(Fork) public extension Engine {
  /// Fork overload: creates a scene while optionally overriding the page size and force-trimming
  /// each clip to fit within `maxTrimmingDuration`.
  /// - Parameters:
  ///   - result: The camera result to build the scene from.
  ///   - size: When set, overrides the computed union frame with a fixed page size.
  ///   - maxTrimmingDuration: When set, caps the total scene duration; each clip is trimmed in order
  ...
  func createScene(
    ...
    case let .capture(captures):
      try await createSceneFromCaptures(captures, size: size, maxTrimmingDuration: maxTrimmingDuration)
    }
  }
}
...
private extension Engine {
  ...
  ///   - size: When set, overrides the computed union frame with a fixed page size.
  ///   - maxTrimmingDuration: When set, caps the total duration added to the scene.
  ...
  ///
  /// - Parameters:
  ///   - captures: The captures to render.
  ///   - size: When set, overrides the computed union frame with a fixed page size.
  ///   - maxTrimmingDuration: When set, caps the total duration added to the scene.
  func createSceneFromCaptures(
    _ captures: [Capture],
    size: CGSize? = nil,
    maxTrimmingDuration: Double? = nil
  ) async throws {
    ...
    if captures.count == 1 {
      ...
      case let .video(recording):
        try await createSceneFromRecordings([recording], size: size, maxTrimmingDuration: maxTrimmingDuration)
        return
      }
    }
  ...
  ///   - maxTrimmingDuration: When set, the total duration is capped; each capture is trimmed in order until the budget
  ///   is exhausted.
  ...
  func addCaptures(
    ...
    var offset: Double = 0
    var remainingTrimmingDuration = maxTrimmingDuration
    ...
    for capture in captures {
      var captureDuration = capture.duration.seconds
      if let remainingTrimmingDuration {
        captureDuration = min(remainingTrimmingDuration, captureDuration)
      }
      switch capture {
      ...
      if let remaining = remainingTrimmingDuration {
        remainingTrimmingDuration = remaining - captureDuration
      }
    }
    return offset
  }
